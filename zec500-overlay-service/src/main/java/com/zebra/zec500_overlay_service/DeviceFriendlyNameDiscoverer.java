...
public class DeviceFriendlyNameDiscoverer {
    ...
    public DeviceFriendlyNameDiscoverer(Context appctx) {
        localContext = appctx;
       // globalFragment = fragment;

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            Log.i("DeviceFriendlyNameDiscoverer", "Initializing WifiP2pManager");
            manager = (WifiP2pManager) localContext.getSystemService(Context.WIFI_P2P_SERVICE);
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            Log.i("DeviceFriendlyNameDiscoverer", "Initializing WifiP2pManager.Channel");
            channel = manager.initialize(localContext, getMainLooper(), null);
        }
    ...
    public boolean hasPermissions() {
        ...
        permissionsGranted = ContextCompat.checkSelfPermission(localContext, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED
                && ActivityCompat.checkSelfPermission(localContext, Manifest.permission.NEARBY_WIFI_DEVICES) == PackageManager.PERMISSION_GRANTED;
    ...
    public void initiateDiscovery() {
        if (hasPermissions()) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
                ...
                if (ContextCompat.checkSelfPermission(localContext, Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED || ActivityCompat.checkSelfPermission(localContext, Manifest.permission.NEARBY_WIFI_DEVICES) != PackageManager.PERMISSION_GRANTED) {
                    return;
                }
    ...
    public void requestDeviceInfo(DeviceInfoCallback callback) {
